...
using ViSCARecorder.Recorder22_;
using Face = ViSCARecorder.Recorder22_.Face;
using Record = ViSCARecorder.Recorder22_.Record;
using Capture = ViSCARecorder.Recorder22_.Capture;
using UnityEngine.UI;
using Unity.VisualScripting;

namespace ViSCARecorder
{
    public class Recorder22 : MonoBehaviour
    {
        // Shared (public).
        ...
        private void Start_Texts()
        {
            ...
            string debugOutputString =
                "- begin ViSCARecorder.Recorder22.Start\n"
                + "\n"
                + "record: {...}\n"
                + "\n"
                + "- end ViSCARecorder.Recorder22.Start\n"
            ;
        ...
        private void FixedUpdate_Texts()
        {
            ...
            if (text_DebugUpdateCountdown <= time_FixedDeltaTimeSeconds)
            {
                ...
                string debugOutputString =
                    $"- begin ViSCARecorder.Recorder22.FixedUpdate\n"
                    + $"\n"
                    + $"{recordString}\n"
                    + $"\n"
                    + $"- end ViSCARecorder.Recorder22.FixedUpdate\n"
                ;
        ...
        private void FixedUpdate_Record()
        {
            ...
            if (record_UpdateCountdown < 0f)
            {
                ...
                record_Current.recorder_name = nameof(Recorder22);
