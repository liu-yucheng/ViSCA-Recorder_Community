...
namespace ViSCARecorder.Recorder15NameSpace
{
    ...
    public class Capture_EncodingTask
    {
        ...
        public List<NativeArray<byte>> pixelsList = new();
        public List<Capture_EncodingTask_TextureInfo> infoList = new();
        ...
        public void Start()
        {
            int count = Math.Min(paths.Count, pixelsList.Count);

            for (int index = 0; index < count; index += 1)
            {
                Capture_EncodingTask_TextureInfo info = infoList[index];
                NativeArray<byte> pixels = pixelsList[index];
                
                NativeArray<byte> image = ImageConversion.EncodeNativeArrayToJPG(
                    pixels,
    ...
    public class Capture_EncodingTask_TextureInfo
    {
        public int width;
        public int height;
    }
    ...
    public class Capture_SavingTask
    {
        ...
        public void Start()
        {
            int count = Math.Min(paths.Count, bytesList.Count);
