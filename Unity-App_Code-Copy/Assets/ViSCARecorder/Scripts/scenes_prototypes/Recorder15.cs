...
using ViSCARecorder;
using ViSCARecorder.Recorder15NameSpace;

namespace ViSCARecorder
{
    public class Recorder15 : MonoBehaviour
    {
        // Shared (public).
        ...
        private void Start_Texts()
        {
            ...
            string debugOutputString =
                "- begin ViSCARecorder.Recorder15.Start\n"
                + "\n"
                + "record: {...}\n"
                + "\n"
                + "- end ViSCARecorder.Recorder15.Start\n"
            ;
        ...
        private void FixedUpdate_Texts()
        {
            ...
            string debugOutputString =
                $"- begin ViSCARecorder.Recorder15.FixedUpdate\n"
                + $"\n"
                + $"{recordString}\n"
                + $"\n"
                + $"- end ViSCARecorder.Recorder15.FixedUpdate\n"
            ;
        ...
        private void FixedUpdate_Capture()
        {
            ...
            if (capture_EncodeSaveCountdown < 0f)
            {
                ...
                ThreadStart start = new(encodingTask.Start);
                encodingTask.thread = new(start);
                encodingTask.thread.Start();
        ...
        private void OnApplicationFocus_Capture(bool focus)
        {
            if (!focus)
            {
                StartCoroutine(OnApplicationFocus_Capture_ProcessAllTasks());
            }
        }
        ...
        private void Start_Capture_EndCameraRendering(ScriptableRenderContext context, Camera camera)
        {
            if (camera == capture_RecordCamera)
            {
                if (capture_Countdown < 0f)
                {
                    ...
                    capture_RecordFileName = $"{folderTimeMSInt:d8}.jpg";
                    string path = capture_RecordFileName;
                    __.ReplaceInvalidCharsWith("_", ref path);
                    path = Path.Combine(capture_RecordFolderName, path);
                    capture_EncodingTask.paths.Add(path);
                    ...
                    NativeArray<byte> pixels = texture.GetPixelData<byte>(0);
                    capture_EncodingTask.pixelsList.Add(pixels);

                    Capture_EncodingTask_TextureInfo info = new()
                    {
                        width = texture.width,
                        height = texture.height
                    };

                    capture_EncodingTask.infoList.Add(info);
                }
            }
        }
        ...
        private void FixedUpdate_Capture_ProcessEncodingTasks()
        {
            ...
            foreach (Capture_EncodingTask encodingTask in oldEncodingTasks)
            {
                if (encodingTask.completed)
                {
                    ...
                    savingTask.paths = encodingTask.paths;
                    savingTask.bytesList = encodingTask.bytesList;
                    ThreadStart threadStart = new(savingTask.Start);
        ...
        private IEnumerator OnApplicationFocus_Capture_ProcessAllTasks()
        {
