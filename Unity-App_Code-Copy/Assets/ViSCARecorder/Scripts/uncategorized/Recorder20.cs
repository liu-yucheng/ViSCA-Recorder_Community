...
using ViSCARecorder.Recorder20_;
using Face = ViSCARecorder.Recorder20_.Face;
using Record = ViSCARecorder.Recorder20_.Record;
using Capture = ViSCARecorder.Recorder20_.Capture;
using UnityEngine.UI;
using Unity.VisualScripting;

namespace ViSCARecorder
{
    public class Recorder20 : MonoBehaviour
    {
        // Shared (public).
        ...
        public GameObject text_DebugWindowObject;
        public GameObject text_DebugOutputObject;
        public GameObject text_DebugToggleButtonObject;
        ...
        private Text text_DebugOutputUIText;
        private Button text_DebugToggleButton;
        private float text_InfoUpdateIntervalSeconds = 0.033_333_333f; // 30 Hz.
        private float text_InfoUpdateCountdown;
        private Text text_InfoOutputUIText;
        private Color text_InfoNoSicknessColor = new(0f, 1f, 0f, 1f);
        private Color text_InfoWeakSicknessColor = new(1f, 1f, 0f, 1f);
        private Color text_InfoSicknessColor = new(1f, 0f, 0f, 1f);
        ...
        private Text record_StatusUIText;
        private Color record_StatusOKColor = new(0f, 1f, 0f, 1f);
        private Color record_StatusWarningColor = new(1f, 1f, 0f, 1f);
        private Color record_StatusCriticalColor = new(1f, 0f, 0f, 1f);
        ...
        private Text capture_StatusUIText;
        private Color capture_StatusOKColor = new(0f, 1f, 0f, 1f);
        private Color capture_StatusWarningColor = new(1f, 1f, 0f, 1f);
        private Color capture_StatusCriticalColor = new(1f, 0f, 0f, 1f);
        ...
        private void FixedUpdate_Time()
        {
            ...
            if (time_UpdateCountdown <= time_FixedDeltaTimeSeconds)
            {
                ...
                record_Current.timestamp.fixed_delta_time_seconds = time_FixedDeltaTimeSeconds;
            }
        }
        ...
        private void Start_Texts()
        {
            ...
            text_DebugOutputUIText = text_DebugOutputObject.GetComponent<Text>();

            string debugOutputString =
                "- begin ViSCARecorder.Recorder20.Start\n"
                + "\n"
                + "record: {...}\n"
                + "\n"
                + "- end ViSCARecorder.Recorder20.Start\n"
            ;

            text_DebugOutputUIText.text = debugOutputString;
            text_DebugToggleButton = text_DebugToggleButtonObject.GetComponent<Button>();
            text_DebugToggleButton.onClick.AddListener(Start_Texts_ToggleDebug);

            text_InfoUpdateCountdown = text_InfoUpdateIntervalSeconds;
            text_InfoOutputUIText = text_InfoOutputObject.GetComponent<Text>();
            ...
            text_InfoOutputUIText.text = timeOutputString;
            text_InfoOutputUIText.color = text_InfoNoSicknessColor;
        }
        ...
        private void FixedUpdate_Texts()
        {
            ...
            if (text_DebugUpdateCountdown <= time_FixedDeltaTimeSeconds)
            {
                ...
                string debugOutputString =
                    $"- begin ViSCARecorder.Recorder20.FixedUpdate\n"
                    + $"\n"
                    + $"{recordString}\n"
                    + $"\n"
                    + $"- end ViSCARecorder.Recorder20.FixedUpdate\n"
                ;

                text_DebugOutputUIText.text = debugOutputString;
            }

            text_InfoUpdateCountdown -= time_FixedDeltaTimeSeconds;

            if (text_InfoUpdateCountdown <= time_FixedDeltaTimeSeconds)
            {
                text_InfoUpdateCountdown = text_InfoUpdateIntervalSeconds;
                ...
                {
                    textColor = text_InfoNoSicknessColor;
                }
                else if (record_Current.sickness.reported <= 0.5f)
                {
                    textColor = text_InfoWeakSicknessColor;
                }
                else  // else if (record_Current.sickness.reported <= 1f)
                {
                    textColor = text_InfoSicknessColor;
                }

                text_InfoOutputUIText.color = textColor;
            }
        }
        ...
        private void Start_Record()
        {
            ...
            record_StatusUIText = record_StatusObject.GetComponent<Text>();
            record_StatusUIText.color = record_StatusWarningColor;
            Start_Record_UpdateStatusText();
        }
        ...
        private void FixedUpdate_Record()
        {
            ...
            if (record_UpdateCountdown < 0f)
            {
                ...
                float recordDeltaTimeSeconds = time_Seconds - record_PreviousRecordSeconds;
                record_Current.timestamp.record_delta_time_seconds = recordDeltaTimeSeconds;
        ...
        private void OnApplicationFocus_Record(bool focus)
        {
            if (!focus)
            {
                record_StatusUIText.color = record_StatusCriticalColor;
            }

            OnApplicationFocus_Record_CompleteAllTasks(focus);

            if (!focus)
            {
                record_StatusUIText.color = record_StatusOKColor;
        ...
        {
            OnApplicationFocus_Record_CompleteAllTasks(!pause);
        }

        private void OnDestroy_Record()
        {
            OnApplicationFocus_Record_CompleteAllTasks(false);
        }
        ...
        private void Start_Capture()
        {
            ...
            capture_StatusUIText = capture_StatusObject.GetComponent<Text>();
            capture_StatusUIText.color = capture_StatusWarningColor;
            Start_Capture_UpdateStatusText();
        }
        ...
        private void OnApplicationFocus_Capture(bool focus)
        {
            if (!focus)
            {
                capture_StatusUIText.color = capture_StatusCriticalColor;
            }

            OnApplicationFocus_Capture_CompleteAllTasks(focus);

            if (!focus)
            {
                capture_StatusUIText.color = capture_StatusOKColor;
        ...
        {
            OnApplicationFocus_Capture_CompleteAllTasks(!pause);
        }

        private void OnDestroy_Capture()
        {
            OnApplicationFocus_Capture_CompleteAllTasks(false);
        }
        ...
        private void Start_Texts_ToggleDebug()
        {
            text_DebugWindowObject.SetActive(!text_DebugWindowObject.activeInHierarchy);
        }
        ...
        private void Start_Record_UpdateStatusText()
        {
            record_StatusUIText.text = $"Recording tasks: {record_Task.subtasks.Count}";
        }

        private void OnApplicationFocus_Record_CompleteAllTasks(bool focus)
        {
            if (!focus)
            {
        ...
        private void Start_Capture_UpdateStatusText()
        {
            capture_StatusUIText.text = $"Capturing tasks: {capture_Task.subtasks.Count}";
        }

        private void OnApplicationFocus_Capture_CompleteAllTasks(bool focus)
        {
            if (!focus)
            {
