...
using ViSCARecorder.Recorder23_;
using Face = ViSCARecorder.Recorder23_.Face;
using Record = ViSCARecorder.Recorder23_.Record;
using Capture = ViSCARecorder.Recorder23_.Capture;
using UnityEngine.UI;
using Unity.VisualScripting;

namespace ViSCARecorder
{
    public class Recorder23 : MonoBehaviour
    {
        // Shared (public).
        ...
        private void Start_Texts()
        {
            ...
            string debugOutputString =
                "- begin ViSCARecorder.Recorder23.Start\n"
                + "\n"
                + "record: {...}\n"
                + "\n"
                + "- end ViSCARecorder.Recorder23.Start\n"
            ;
        ...
        private void FixedUpdate_Texts()
        {
            ...
            if (text_DebugUpdateCountdown <= time_FixedDeltaTimeSeconds)
            {
                ...
                string debugOutputString =
                    $"- begin ViSCARecorder.Recorder23.FixedUpdate\n"
                    + $"\n"
                    + $"{recordString}\n"
                    + $"\n"
                    + $"- end ViSCARecorder.Recorder23.FixedUpdate\n"
                ;
        ...
        private void FixedUpdate_Record()
        {
            ...
            if (record_UpdateCountdown < 0f)
            {
                ...
                record_Current.recorder_name = nameof(Recorder23);
