...
namespace ViSCARecorder.Recorder13NameSpace
{
    ...
    public class Capture_SaveCapturesContext
    {
        public static void SaveCaptures(
            in List<byte[]> recordBytesList,
            in List<string> recordFilePaths
        )
        {
            int count;

            if (recordBytesList.Count < recordFilePaths.Count)
            {
                count = recordBytesList.Count;
            }
            else
            {
                count = recordFilePaths.Count;
            }

            for (int index = 0; index < count; index += 1)
            {
                byte[] bytes = recordBytesList[index];
                string path = recordFilePaths[index];
                File.WriteAllBytes(path, bytes);
            }
        }

        private List<byte[]> recordBytesList;
        private List<string> recordFilePaths;

        public Capture_SaveCapturesContext(
            in List<byte[]> recordBytesList,
            in List<string> recordFilePaths
        )
        {
            this.recordBytesList = recordBytesList;
            this.recordFilePaths = recordFilePaths;
        }

        public void Start()
        {
            SaveCaptures(recordBytesList, recordFilePaths);
        }
    }
