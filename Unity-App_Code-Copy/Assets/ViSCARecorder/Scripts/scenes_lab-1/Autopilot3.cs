...
namespace ViSCARecorder
{
    ...
    public class Autopilot3 : MonoBehaviour
    {
        // begin public fields
        ...
        private float Autopilot_Refresh_Interval = 0.011_111_111f; // 90 Hz.
        ...
        private float Waypoint_ReachThreshold_Meters = 3f;
        ...
        private float Waypoint_ForceNext_Interval = 20f;
        private float Waypoint_ForceNext_Countdown = 20f;
        private bool Waypoint_ForceNext_Needed = false;

        private Vector2 ThrottleBrake_Range_Nominal = new(-1f, 1f);
        private Vector2 ThrottleBrake_Range_Idle = new(-0.5f, 0.5f);
        private Vector2 ThrottleBrake_Range_OverSteer = new(-0.65f, 0.65f);
        ...
        private Vector2 SteeringWheelRange_Nominal = new(-0.8f, 0.8f);
        private Vector2 SteeringWheelRange_Idle = new(-0.2f, 0.2f);
        private Vector2 SteeringWheelRange_OverSteer_Inverse = new(-0.3f, 0.3f);
        private Vector2 SteeringWheelRange_Current = new(-1f, 1f);
        
        private float PathCreate_PatrolRadius_Meters = 150f;
        private float PathCreate_SamplePosition_MaxDistance_Meters = 150f;
        ...
        private Recorder25_.Record.Specials Specials_Instance;
        ...
        void FixedUpdate()
        {
            ...
            Waypoint_ForceNext_Countdown -= Time.fixedDeltaTime;

            if (Waypoint_ForceNext_Countdown < Time.fixedDeltaTime)
            {
                Waypoint_ForceNext_Countdown = Waypoint_ForceNext_Interval;
                Waypoint_ForceNext_Needed = true;
            }
        ...
        public void Specials_Index_Reset(int Random_Specials_Index_)
        {
            ...
            Recorder25_.Record.SpecialsStandards.CreateSpecialsByIndex(
                Specials_Index,
        ...
        private void Waypoint_Positions_Manage()
        {
            ...
            else
            {
                ...
                if (
                    Vector3.Distance(Position_Vehicle_XZ, Position_Waypoint_XZ) < Waypoint_ReachThreshold_Meters
                    || Waypoint_ForceNext_Needed
                )
                {
                    Waypoint_ForceNext_Needed = false;
                    Waypoint_ForceNext_Countdown = Waypoint_ForceNext_Interval;
        ...
        private void Specials_Path_Waypoints_Draw()
        {
            for (int Index_ = 0; Index_ < Waypoint_Positions.Count; Index_ += 1)
            {
                ...
                Vector3 Position_ = Waypoint_Positions[Index_];
                Position_.y = Specials_PositionY_AboveTerrain;
                Position_AtPositionAboveTerrain_OnSurface_Find(Position_, out Position_);
                Position_.y += 0.25f;
                
                Quaternion Rotation_ = Quaternion.Euler(
                    0f, 
                    
                    360f * (
                        (float)Index_
                        / (float)(Waypoint_Positions.Count + 1)
                    ), 
